...
class MediaSortedManyToManyField(_ManyToManyField):
    '''
    ...
    def __init__(self, to, sorted=True, **kwargs):
        ...
        super(MediaSortedManyToManyField, self).__init__(to, **kwargs)
        if self.sorted:
    ...
    def deconstruct(self):
        ...
        name, path, args, kwargs = super(MediaSortedManyToManyField, self).deconstruct()
    ...
    def contribute_to_class(self, cls, name, **kwargs):
        if not self.sorted:
            return super(MediaSortedManyToManyField, self).contribute_to_class(cls, name, **kwargs)
    ...
    def formfield(self, **kwargs):
        ...
        return super(MediaSortedManyToManyField, self).formfield(**defaults)
...
if south is not None and 'south' in settings.INSTALLED_APPS:
    ...
    add_introspection_rules(
        [(
            (MediaSortedManyToManyField,),
            [],
            {"sorted": ["sorted", {"default": True}]},
        )],
        [r'^mediasortedm2m\.fields\.MediaSortedManyToManyField']
    )
    ...
    class AddM2M(south.creator.actions.AddM2M):
        ...
        def console_line(self):
            if isinstance(self.field, MediaSortedManyToManyField) and self.field.sorted:
